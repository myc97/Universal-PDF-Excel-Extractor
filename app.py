...
readme = """# NEET Universal PDF → Excel Extractor
...
This app combines the two supplied extraction approaches:

1. **Table extraction**
   - Uses `pdfplumber` when the PDF contains a real table structure.
2. **Fixed-position extraction**
   - Uses PyMuPDF (`fitz`) to preserve searchable-PDF text alignment.
   - The Quota field is sliced from the same character position used by the supplied fixed-layout extractor.
3. **Scanned PDF OCR**
   - Converts one page at a time with Poppler/pdf2image.
   - OCR uses Tesseract.
4. **College context**
   - Detects a college/institute heading above a header/table.
   - Carries the current college context across pages until a new college heading is detected.
5. **Excel output**
   - Removes exact duplicate records.
   - Sorts by PDF page and Sr. No.
   - Freezes the header, enables filters and sets useful column widths.
   - Adds an Extraction Summary sheet.
...
## Important Quota handling

For the supplied fixed-alignment layout, the important character boundaries are:
